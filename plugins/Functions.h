 ...
 */
  ...
  public:
    ...
    inline double getGEO(const T & parval){
      //inline double getGEO(const double & pt, const double & eta, const T & parval){
      return parval[0];
    }

    inline double getMS(const double & eta, const T & parval){
    ...
    {
      return pt*getGEO(parval) + getMS(eta,parval);
    }
  ...
  public:
    ...
    inline double getGEO(const double & eta, const T & parval){
      // geometrical term
      double qGEO(0);
      if ( eta < parval[0] ) {
    ...
    inline double centralParabola(const double & eta, const T & parval){
      return parval[4] + parval[5]*eta*eta;
    }

    inline double middleParabola(const double & eta, const T & parval){
      return parval[15] + parval[16]*eta*eta;
    }

    inline double leftParabola(const double & eta, const T & parval){
      return parval[6] + parval[7]*(eta-parval[0])*(eta-parval[0]);
    }

    inline double rightParabola(const double & eta, const T & parval){
      return parval[8] + parval[9]*(eta-parval[3])*(eta-parval[3]);
    }

    inline double leftLine(const double & eta, const T & parval){
      double x_1 = parval[13];
      double y_1 = middleParabola(parval[13], parval);
      double x_2 = parval[0];
      double y_2 = leftParabola(parval[0], parval);
      return( (eta - x_1)*(y_2 - y_1)/(x_2 - x_1) + y_1 );
    }

    inline double rightLine(const double & eta, const T & parval){
      double x_1 = parval[14];
      double y_1 = middleParabola(parval[14], parval);
      double x_2 = parval[3];
      double y_2 = rightParabola(parval[3], parval);
      return( (eta - x_1)*(y_2 - y_1)/(x_2 - x_1) + y_1 );
    }


    inline double getMSC(const double & eta, const T & parval){
      // MSC term
      double qMSC(0);
      if ( eta < parval[0] ) {
	qMSC = leftParabola(eta,parval);
      } else if ( parval[0] <= eta && eta < parval[13] ) {
	qMSC = leftLine(eta,parval);
      } else if ( parval[13] <= eta && eta < parval[1] ) {
	qMSC = middleParabola(eta,parval);
      } else if ( parval[1] <= eta && eta < parval[2] ) {
	qMSC = centralParabola(eta,parval);
      } else if ( parval[2] <= eta && eta < parval[14] ) {
	qMSC = middleParabola(eta,parval);
      } else if ( parval[14] <= eta && eta < parval[3] ) {
	qMSC = rightLine(eta,parval);
      } else {
	qMSC = rightParabola(eta,parval);
      }
      return qMSC;
    }
    ...
    virtual double sigmaPt(const double & pt, const double & eta, const T & parval)
    {
      double qGEO = getGEO(eta,  parval);
      double qMSC = getMSC(eta,  parval);
